...
const MainContent: React.FC<MainContentProps> = ({ selectedFile, searchTerm, folders }) => {
  const getAllFiles = (folders: Folder[]): FileItem[] =>
    folders.flatMap((f) => [...(f.files ?? []), ...(f.subfolders ? getAllFiles(f.subfolders) : [])])

  const searchResults =
    searchTerm.trim() === ""
      ? []
      : getAllFiles(folders).filter((file) => file.name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  if (!selectedFile) {
    return (
      <div className="flex flex-1 items-center justify-center p-4">
        <div className="text-center max-w-sm sm:max-w-md p-4 sm:p-5 bg-white rounded-xl shadow-lg border border-gray-100">
          <div className="bg-gradient-to-br from-blue-100 to-indigo-100 w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 rounded-full mx-auto mb-4 flex items-center justify-center">
            <FileText className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-blue-500" />
          </div>
          <h2 className="text-lg sm:text-xl font-bold text-gray-800 mb-2">Document Viewer</h2>
          <p className="text-gray-600 text-sm mb-4">Select a file from the sidebar to preview its contents</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <h2 className="text-sm sm:text-base lg:text-lg font-bold text-gray-800 truncate">{displayName}</h2>
                <Button
                  variant="ghost"
                  size="icon"
                ...
                >
                  <Star
                    className={`w-3 h-3 sm:w-4 sm:h-4 ${isFavorite ? "fill-yellow-400 stroke-yellow-400" : "stroke-gray-400"}`}
                  />
                </Button>
              </div>
            ...
            <Document
              ...
              error={
                <div className="py-6 sm:py-8 text-center px-4">
                  ...
                  <h3 className="text-sm sm:text-base font-semibold text-red-600 mb-1">Failed to load document</h3>
                  <p className="text-gray-600 text-xs sm:text-sm max-w-md mx-auto">
                    The PDF file couldn't be loaded. Please check the file format or try again later.
                  </p>
...
{Array.from(new Array(numPages), (_, index) => (
  <Page
    key={`page_${index + 1}-${containerWidth}`} // force re-render when containerWidth changes
    pageNumber={index + 1}
    width={containerWidth ? Math.min(containerWidth - 32, 800) : Math.min(window.innerWidth - 32, 600)}
        ...
        <section className="flex-1 overflow-auto px-3 py-3 sm:px-4 sm:py-4 lg:px-6 bg-gradient-to-br from-gray-50 to-blue-50">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-center mb-4 p-3 sm:p-4 bg-white rounded-xl shadow-sm gap-2 sm:gap-3">
              ...
              <Badge variant="outline" className="bg-blue-50 text-blue-600 self-start sm:ml-auto">
            ...
            {searchResults.length > 0 ? (
              ...
              <div className="text-center py-6 sm:py-8 px-4">
                ...
                <h3 className="text-base sm:text-lg font-semibold text-gray-600 mb-2">No documents found</h3>
                <p className="text-gray-500 text-sm">Try adjusting your search term or browse different categories.</p>
              </div>
            )}
          </div>
        </section>
